...
export async function GET(request: Request) {
  try {
    ...
    const description =
      searchParams.get("description") || "A blog by Nii Tettey";

    return new ImageResponse(
      (
        <div tw="h-full w-full flex items-start justify-start bg-yellow-50 relative">
          <div tw="flex flex-col  p-20">
            <h1 tw="text-7xl font-bold text-left text-gray-900">{title}</h1>
            <p tw="text-xl font-bold text-left">{description}</p>
          </div>
